        ...
        try {
            with(infoApi.getInfo()) {
                when {
                    ...
                    error != null -> Err(Exception(error)) // DONE: Convert to error
                    else -> Err(Exception("Unknown API Error")) // DONE: Convert to error
                }
            }
        } catch (throwable: Throwable) {
            Timber.e(throwable, "InfoRepositoryImpl")
            Err(Exception("API Failure", throwable)) // Done: Convert to error
        }
}
